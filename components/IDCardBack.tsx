...
const IDCardBack: React.FC<IDCardBackProps> = ({ record, companyName, companyLogo, companyWebsite, provostSignature }) => {
  ...
  return (
    <div
      className="w-[320px] h-[512px] bg-white rounded-xl shadow-lg p-4 flex flex-col items-center justify-between font-sans text-black"
    ...
    >
      ...
      <main className="flex-grow flex flex-col items-center text-center px-4 py-4 w-full justify-around">
        <div>
            ...
            <p className="text-[11px] font-medium text-black" style={{ color: '#000000' }}>This card is the property of</p>
            <h2 className="text-sm font-bold mt-1 text-black uppercase tracking-wide leading-tight px-2" style={{ color: '#000000' }}>
              {renderCompanyName(companyName)}
            </h2>

            <p className="text-[11px] mt-4 text-black" style={{ color: '#000000' }}>
        ...
        <div className="flex flex-col items-center w-full">
            ...
            <div className="w-48 border-t border-black mt-1" style={{ borderColor: '#000000' }}></div>
            <p className="text-[11px] mt-1 text-black font-medium" style={{ color: '#000000' }}>Provost Signature</p>
        </div>

        {/* QR Code */}
        <div title="Scan to save contact details (vCard)" className="cursor-help">
          <div className="p-1.5 bg-white border border-gray-100 rounded-lg shadow-md">
            <QRCodeCanvas value={vCardData} size={115} includeMargin={true} level="L" />
          </div>
        </div>
      </main>
      ...
      <div className="w-full">
        ...
        <p className="text-center text-black text-sm font-semibold mt-2" style={{ color: '#000000' }}>{companyWebsite}</p>
      </div>
    </div>
  );
};
